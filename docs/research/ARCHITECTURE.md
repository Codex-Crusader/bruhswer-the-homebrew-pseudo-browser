...
5. The file lands in `%LOCALAPPDATA%\...\Quarantine\<session>\`, with a **non-executing extension appended**, its `Zone.Identifier` set to Zone 3 (Internet) so SmartScreen and Defender engage on any later attempt to open it, and a host-computed SHA-256.
   ...
   > **What shipped is different (2026-09-25).** This is the Stage 1 design. The shipped build has no guest VM. Edge downloads straight into quarantine. bruhswer does not append an extension, compute a SHA-256 or write a `Zone.Identifier` on the quarantined file. Edge writes the stream. One quarantined file, measured with `Get-Content -Stream Zone.Identifier`, carried `ZoneId=3`. bruhswer writes and reads back `ZoneId=3` on every EXPORTED copy, and refuses the export if the destination drive cannot hold the stream. See `downloads/quarantine.py::mark_of_the_web`.
